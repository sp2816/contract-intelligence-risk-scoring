      ...
      <div className="mx-auto flex h-16 w-full items-center justify-between px-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-3">
          ...
          {user && (
            <button
              ...
              className={`hidden lg:inline-flex items-center justify-center rounded-xl p-2 transition ${
                isLight ? 'text-slate-500 hover:bg-slate-100 hover:text-slate-900' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
              }`}
        ...
        <div className="flex items-center gap-4">
          {user ? (
            <>
              {/* Search */}
              <div className={`hidden md:flex items-center rounded-full px-3 py-1.5 border focus-within:border-brand-500/50 transition-colors ${
                isLight ? 'bg-slate-100 border-slate-200' : 'bg-slate-800/50 border-slate-700/50'
              }`}>
                <Search className={`h-4 w-4 ${isLight ? 'text-slate-400' : 'text-slate-400'}`} />
                <input
                  type="search"
                  placeholder="Search contracts..."
                  className={`bg-transparent border-none outline-none text-sm ml-2 w-48 placeholder:text-slate-500 ${
              ...
              {/* Notifications */}
              <button className={`relative p-2 transition rounded-full ${
                isLight ? 'text-slate-500 hover:bg-slate-100 hover:text-slate-900' : 'text-slate-400 hover:text-white hover:bg-slate-800'
              }`}>
